...
public class FilesystemCDNContainer extends AbstractCDNContainer {
	...
	public void preStart() throws Exception {		
		super.preStart();
		globalBaseDirectory = InstanceConfig.getInstance().getConfig().getString("visualizations.path")+"/web";
	}
	...
	public void doActivate(ActorRef sender, DeployAction action, File deploymentDir, String filename) {
		...
		cmd.add("/bin/tar");		
		cmd.add("-xf");
		cmd.add(deploymentDir.getAbsolutePath()+"/"+getCDNArchive(filename));
		cmd.add("-C");
		cmd.add(".");
	...
	void publishAction(DeployAction msg) throws AppException {
	  ...
	  public void doWipe(DeployAction action, File deploymentDir) {
			...
			result(action, DeployPhase.REPORT_WIPE_CDN, true, "");
		}
