...
import server from "./server";
	...
	service.getRecords = function(aps, properties, fields) {
		...
		return server.post(jsRoutes.controllers.Records.getRecords().url, data);
	};
	...
	service.getInfos = function(aps, properties, summarize) {
		...
		return server.post(jsRoutes.controllers.Records.getInfo().url, data);
	};
	
	service.getRecord = function(recordId) {
		var data = {"_id": recordId };		
		return server.post(jsRoutes.controllers.Records.get().url, data);
	};
	
	service.unshare = function(aps, records, type, query) {
	  if (! Array.isArray(aps)) aps = [ aps ];
	  if (records != null && ! Array.isArray(records)) records = [ records ];
	  var data = { records:records, started:[], stopped:aps, type:type, query:query };		
	  return server.post(jsRoutes.controllers.Records.updateSharing().url, data);
	};
	...
	service.shareSpaceWithCircle = function(fromSpace, toConsent) {
		...
		return server.post(jsRoutes.controllers.Records.share().url, data);
	};
