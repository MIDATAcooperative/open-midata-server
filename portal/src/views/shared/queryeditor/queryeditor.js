...
.controller('QueryEditorCtrl', ['$scope', '$state', 'status', 'session', 'formats', '$translate', 'apps', 'server', 'labels', '$translatePartialLoader', function($scope, $state, status, session, formats, $translate, apps, server, labels, $translatePartialLoader) {
	...
	$scope.reload = function() {
		...
		if ($state.current.data.mode == "study") {
			...
			$scope.mode = "app";
			$scope.status.doBusy(apps.getApps({ "_id" : $state.params.appId }, ["creator", "developerTeam", "filename", "name", "description", "tags", "targetUserRole", "spotlighted", "type","accessTokenUrl", "authorizationUrl", "consumerKey", "consumerSecret", "tokenExchangeParams", "defaultQuery", "defaultSpaceContext", "defaultSpaceName", "previewUrl", "recommendedPlugins", "requestTokenUrl", "scopeParameters","secret","redirectUri", "url","developmentServer","version","i18n","status", "resharesData", "allowsUserSearch", "pluginVersion", "requirements", "termsOfUse", "orgName", "publisher", "unlockCode", "writes", "icons", "apiUrl", "noUpdateHistory","pseudonymize"]))
			.then(function(data) { 
